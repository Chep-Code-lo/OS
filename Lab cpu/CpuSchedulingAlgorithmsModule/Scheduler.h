...
inline void print_gantt(const Processes &p, const vector<GanttSeg> &segs) {
    ...
    cout << "\t|";
    for (auto &s : segs) {
        int w = (s.end - s.start) * 2;
        int il = (int)p[s.idx].id.size();
        int pl = max(0, (w - il) / 2);
        int pr = max(0, w - il - pl);
    ...
    cout << "\t";
    for (auto &s : segs)
        cout << left << setw((s.end - s.start) * 2 + 1) << s.start;
    cout << segs.back().end << "\n";
}
...
inline void print_table(const Processes &p, const vector<GanttSeg> &segs) {
    ...
    auto executed_before = [&](int i, int t) {
        int ex = 0;
        for (auto &s : segs) {
            if (s.idx != i) continue;
            int e = min(s.end, t);
    ...
    auto remaining_at = [&](int i, int t) {
        return max(0, p[i].burst - executed_before(i, t));
    };

    auto running_at = [&](int t) -> int {
        for (auto &s : segs)
            if (s.start == t) return s.idx;
        return -1;
    };
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (int t : times) {
            ...
            int rem = remaining_at(i, t);
            bool running = (running_at(t) == i);
