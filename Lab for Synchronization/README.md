...
```
P 2 -> 0        ← Producer (thread 2) sản xuất số 0
C 4 <- 0        ← Consumer (thread 4) lấy số 0 ra ngay
P 3 -> 1        ← Producer (thread 3) sản xuất số 1
C 5 <- 1        ← Consumer (thread 5) lấy số 1 ra
P 3 -> 2
P 2 -> 3        ← Cả 2 producer nhét vào buffer liên tiếp
C 5 <- 2        ← 2 consumer lấy ra đồng thời
C 4 <- 3
P 2 -> 4
P 3 -> 5
...
...
| Ký hiệu | Ý nghĩa |
|---------|-------|
| `P <tid> -> <N>` | Producer thread `<tid>` đã **sản xuất** số `<N>` vào buffer |
| `C <tid> <- <N>` | Consumer thread `<tid>` đã **tiêu thụ** số `<N>` từ buffer |

**Quan sát từ output thực tế:**
- Mỗi số chỉ xuất hiện đúng **1 lần** ở P và **1 lần** ở C → không mất, không trùng.
- Khi buffer có chỗ, 2 producer có thể cùng nhét liên tiếp rồi 2 consumer lấy cùng luc.
- Thứ tự FIFO được bảo toàn.
...
```
Thread 5 is WRITING.         ← Writer vào trước (độc quyền)
Thread 5 is DONE WRITING.    ← Writer xong, nhả writeLock
Thread 8 is READING.         ← 4 reader vào cùng luc ngay sau writer
Thread 7 is READING.
Thread 6 is READING.
Thread 9 is READING.         ← Tất cả 4 reader đang đọc song song
Thread 9 is DONE READING.
Thread 6 is DONE READING.
Thread 7 is DONE READING.
Thread 8 is DONE READING.    ← Reader cuối xong, writeLock được nhả
Thread 4 is WRITING.         ← Writer tiếp theo vào (độc quyền)
```
