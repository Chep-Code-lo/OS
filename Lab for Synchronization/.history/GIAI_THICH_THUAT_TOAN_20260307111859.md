...
```
P 1 -> 0        ← Producer (thread id 1) đã đặt số 0 vào buffer
P 2 -> 1        ← Producer khác đặt số 1
C 3 <- 0        ← Consumer lấy số 0 ra
C 4 <- 1        ← Consumer khác lấy số 1 ra
```
...
```
Thread 1 is READING.         ← Reader bắt đầu đọc
Thread 2 is READING.         ← Reader khác cũng đọc đồng thời (hợp lệ)
Thread 3 is READING.         ← Thêm một reader nữa
Thread 1 is DONE READING.    ← Reader 1 xong
Thread 2 is DONE READING.    ← Reader 2 xong
Thread 3 is DONE READING.    ← Reader cuối, writeLock được nhả
Thread 4 is WRITING.         ← Writer vào (độc quyền)
Thread 4 is DONE WRITING.    ← Writer xong, nhả writeLock
Thread 5 is READING.         ← Readers tiếp tục
```
