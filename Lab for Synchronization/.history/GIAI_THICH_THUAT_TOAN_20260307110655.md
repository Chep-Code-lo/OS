# Giải Thích Thuật Toán Đồng Bộ Hóa (Synchronization)
...
**Tham số chạy chương trình:**
```
./bounded_buffer <giây_chạy> <số_producer> <số_consumer> <số_bắt_đầu>
Ví dụ: ./bounded_buffer 10 2 2 0
```

**Đảm bảo:** Mỗi số chỉ được tiêu thụ **đúng một lần** và thứ tự FIFO được duy trì.
...
void pickup_forks(int id) {
    int left = LEFT;   // (id-1+5) % 5
    int right = RIGHT; // (id+1) % 5
    ...
    while ((state[id] == HUNGRY) &&
        ...
        printf("Philosopher %i is hungry and waiting...\n", id);
...
void return_forks(int id) {
    ...
    printf("Philosopher %d signaled philosopher %d...\n", id, left);
