...
inline void MLFQ(Processes &p) {
    ...
    auto enqueueArrivals = [&](int time) {
        for (int i = 0; i < n; i++) {
            if (p[i].arrive_time <= time && remain[i] > 0 && !inQueue[i] && i != curProc) {
    ...
    while (completed < n) {
        ...
        for (int lv = 0; lv < 3; lv++) {
            if (!queues[lv].empty()) { bestLv = lv; break; }
        }
        if (curProc != -1) {
            int curLv = level[curProc];
            if (bestLv != -1 && bestLv < curLv) {
                queues[curLv].push_front(curProc);
