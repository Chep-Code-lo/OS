...
Level 1: quantum = 16
Level 2: quantum = 0 nghĩa là FCFS chạy tới khi xong hoặc preempt bởi queue ưu tiên cao hơn (chạy hết burst)
```
**Ý tưởng:**
```
1. Process mới đến sẽ vào queue theo level hiện tại của nó.
2. Luôn chọn chạy process ở queue có level nhỏ nhất đang không rỗng.
3. Nếu process đang chạy mà có process ưu tiên cao hơn đến (bestLv < curLv) → preempt (đẩy process đang chạy về đầu queue của nó).
4. Hết quantum mà chưa xong → bị hạ level (level++ nếu chưa tới 2) và đưa lại vào queue mới.
```
*Hàm enqueueArrivals(time)*
```
- Quét toàn bộ process.
- Nếu process đã tới (arrive_time <= time), chưa xong (remain > 0), chưa nằm trong queue (!inQueue), và không phải đang chạy (i != curProc) -> đẩy vào queue theo level[i].
=> Mỗi tick cập nhật "ai đã đến hệ thống"
```
```
1.  đưa các process đã tới vào queue theo level,
2.  nếu có process ưu tiên cao hơn thì ngắt process đang chạy,
3.  chọn process ở queue ưu tiên cao nhất để chạy 1 tick,
4.  nếu xong thì tính waiting/completion,
5.  nếu hết quantum thì hạ level và đưa lại vào queue.
```
---
